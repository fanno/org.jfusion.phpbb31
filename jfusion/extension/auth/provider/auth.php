...
namespace jfusion\extension\auth\provider;
...
class auth extends \phpbb\auth\provider\db
{
	...
	public function login($username, $password)
	{
		...
		if ($result['status'] == LOGIN_SUCCESS && empty($JFusionActive)) {
			...
			$joomla->setActivePlugin($this->config['jfusion_extension_jname']);
			$joomla->login($username, $password, $remember);

			if ($this->config['jfusion_extension_redirect_login']) {
...
*/
	...
	public function logout($data, $new_session)
	{
		...
		if (empty($JFusionActive)) {
			...
			$link = null;
			if ($this->config['jfusion_extension_redirect_logout']) {
				//redirect to prevent fatal errors on some servers
				$uri = JURI::getInstance();
				//remove sid from URL
				$query = $uri->getQuery(true);
...
*/
	...
	function startJoomla() {
		...
		$apipath = $this->config['jfusion_extension_apipath'];
